const { createBooking, getBookingsByUser, countBookingsForEvent } = require('../models/bookingModel');
...
const book = async (req, res) => {
  try {
    ...
    if (new Date(event.date_time) < new Date()) {
      return res.status(400).json({ error: 'This event has already started or passed' });
    }

    const currentBookings = await countBookingsForEvent(event_id);
